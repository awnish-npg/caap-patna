                ...
                <div class="panel_s">
                    <div class="panel-body">
                        <div class="panel-header">
                            ...
                            if (has_permission('categories_manage', '', 'create')) {
                            ?>
                            ...
                            ?>
                            <h1>Manage Action Items<span>Here you can view, add, edit and deactivate Action Items and Milestones.</span></h1>
    ...
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <p class="form-instruction add-title">Here you can introduce a new action item by filling in the following fields</p>
                        ...
                        <div class="form-input-field"><input class="labellll-up" type="text" required="" id="issue_name" name="issue_name" value="<?php set_value('milestone_name'); ?>">
                            <label for="issue_name" title="Action Item Name*" data-title="Category Name*"></label>
                            <span>
        ...
        <script>
            ...
            $(function() {
                ...
                $('#issue').on('hidden.bs.modal', function(event) {
                    ...
                    $('#issue input[name="issue_name"]').removeAttr("disabled")
            ...
            function addPrefix(name) {
                ...
                if (areaPrefix != "") {
                    name = `${areaPrefix} - ${name}`;
                }
                return name;
            }

            function createSerializeData(name) {
                var values, index;

                // Get the parameters as an array
                values = $("#issue_form").serializeArray();
                let areaPrefix = "<?php echo @$area_prefix ?>";
                // Find and replace `content` if there
                if (areaPrefix != "") {

                    for (index = 0; index < values.length; ++index) {
                        if (values[index].name == "issue_name") {
                            values[index].value = `${areaPrefix} - ${name}`;
                            break;
                        }
                    }
                }
            ...
            $("#submit_issue").click(function(e) {
                ...
                if (id == "") {
                    let issue_name = addPrefix($('#issue_name').val());
                    var data = {
                        "id": id,
                        "issue_name": issue_name,
                        "issue_name_reg": $('#issue_name').val(),
                    ...
                    var data = createSerializeData($('#issue_name').val());
            ...
            function edit_issue(invoker, id) {
                ...
                let issue_name = $(invoker).data('name');
                if (issue_name.includes(" - ")) {
                    issue_name = issue_name.split(' - ')[1];
                }
                $('#additionalhiddenfields').append(hidden_input('name', $(invoker).data('name')));
                $('#issue input[name="issue_name"]').val(issue_name);
                ...
                var issue_parent_id = strtoarray($(invoker).data('parent'));
                if (issue_parent_id != 0) {
                    $('#issue input[name="issue_name"]').attr("disabled", "disabled");
                } else {
                    $('#issue input[name="issue_name"]').removeAttr("disabled")
                }
