...
function init_admin_auth_assets()
{
    ...
    $CI->app_css->add('waitme-css', 'assets/plugins/waitMe/waitMe.css', $groupName);
    if (is_rtl()) {
    ...
    $CI->app_scripts->add('waitme-js', 'assets/plugins/waitMe/waitMe.js');
}
...
function _init_admin_assets()
{
    ...
    $CI->app_scripts->add('jquery-migrate-js', 'assets/plugins/jquery/jquery-migrate.' . (ENVIRONMENT === 'production' ? 'min.' : 'min.') . 'js');
    ...
    $CI->app_scripts->add(
        'app-js',
        base_url($CI->app_scripts->core_file('assets/js', 'main.js')),
        'admin',
    ...
    add_waitMe_js_assets('admin');

    if (is_rtl()) {
        $CI->app_css->add('bootstrap-rtl-css', 'assets/plugins/bootstrap-arabic/css/bootstrap-arabic.min.css');
    }

    $CI->app_css->add('app-css', base_url($CI->app_css->core_file('assets/css', 'style.css')) );

    if (file_exists(FCPATH . 'assets/css/custom.min.css')) {
        $CI->app_css->add('custom-css', base_url('assets/css/custom.min.css'). '?v=2.4.4', 'admin', ['app-css']);
    }
...
function add_waitMe_js_assets($group = '')
{
    $locale = $GLOBALS['locale'];
    $CI     = &get_instance();

    $CI->app_scripts->add('waitme-js', 'assets/plugins/waitMe/waitMe.js', $group);

    $CI->app_css->add('waitme-css', 'assets/plugins/waitMe/waitMe.css', $group);
}
