...
class Cron_model extends App_Model
{
    ...
    private function tasks_reminders()
    {
        ...
        $this->db->where('projects.status IN  (1,2,4,6)');
        $this->db->where('projects.frozen', '0'); 
        $this->db->where("(reminderone_date =  '".$now."' OR remindertwo_date = '".$now."')");
        $this->db->where('deadline_notified <', 2);
        ...
        foreach ($tasks as $task) {
            if((empty($task['reminderone_notified'] ) || $task['reminderone_notified']!=$now)
                && (empty($task['remindertwo_notified'] ) || $task['remindertwo_notified']!=$now)){

                if($task['deadline_notified'] == 0){
                    $notify_update =    ['deadline_notified' => $task['deadline_notified']+1,
                                        'reminderone_notified' => $task['reminderone_date']];
                }
                if($task['deadline_notified'] == 1){
                    $notify_update =    ['deadline_notified' => $task['deadline_notified']+1,
                                        'remindertwo_notified' => $task['remindertwo_date']];
                }
             //   print_r($task);
                $role = "('at', 'ata')";
                $assignees = $this->projects_model->get_project_members($task['rel_id'], $role);

                $email_data = [];
                if($task['deadline_notified'] == 0){
                    $email_data['reminder_count'] = '1st';
                }
                if($task['deadline_notified'] == 1){
                    $email_data['reminder_count'] = '2nd';
                }
    ...
    public function sendSms($staff_id,$project_id,$number){
        $ch  =  curl_init();
        $timeout  =  30; 
        $due_date=get_project_due_date($project_id);
        $phonenumber=get_staff_phone($staff_id);
        $message=$this->load->view('sms_templates/reminders',['project_id'=>$project_id,'due_date'=>$due_date,'number'=>$number],true);
